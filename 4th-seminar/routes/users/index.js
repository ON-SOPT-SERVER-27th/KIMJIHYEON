...
router.post('/signup', async (req, res) => {
    ...
    if (!email || !password || !userName) {
        ...
        return res.send(util.fail(statusCode.BAD_REQUEST, responseMessage.NULL_VALUE));
    }
...
router.post('/signin', async (req, res) => {
    ...
    try {
        ...
        //4. 비밀번호 확인하기 - 로그인할 email의 salt를 DB에서 가져와서
        // 사용자가 request로 보낸 password와 암호화를 한 후 디비에 저장되어 있는 password와 일치하면 true
...
router.delete('/delete', async (req, res) => {
    // 1. 회원정보 삭제 시 email, password 입력
    // 2. 해당 레코드 정보 지우기
    // 3. 지운 다음 responseMessage.MEMBER_DELETE_SUCCESS
    ...
    try {
        ...
        // 사용자가 request로 보낸 password와 암호화를 한 후 디비에 저장되어 있는 password와 일치하면 true
        // 일치하지 않으면 Miss Match password 반환
        const { id, userName, salt, password: hashedPassword } = userEmail;
        const inputPassword = crypto.pbkdf2Sync(password, salt, 10000, 64, 'sha512').toString('base64');
...
router.put('/modify/:id', async (req, res) => {
    ...
    try {
        ...
        const {email, userName} = req.body; // 3. req.body에서 데이터 가져오기

        //4. request data 확인하기, email, userName data가 모두 없다면 (수정할 데이터가 아무것도 들어오지 않았다면 NullValue 반환
        if (!email && !userName) {
            return res.status(statusCode.BAD_REQUEST).send(util.fail(statusCode.BAD_REQUEST, responseMessage.NULL_VALUE));
        }
